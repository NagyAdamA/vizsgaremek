...
const options = {
    definition: {
        ...
        info: {
            ...
            version: "1.0.0",
            description:
                "REST API for the Vizsgaremek archery score tracking application.\n\n" +
                "Authentication is cookie-based (HttpOnly JWT). Call `POST /api/auth/login` first — " +
                "the cookie is then sent automatically with subsequent requests.",
        },
        ...
        security: [{ cookieAuth: [] }],
        tags: [
            { name: "Auth", description: "Login, logout, email verification, password reset" },
            { name: "Users", description: "User registration and retrieval" },
            { name: "Sessions", description: "Archery training sessions" },
            { name: "Scores", description: "Arrow scores within a session" },
